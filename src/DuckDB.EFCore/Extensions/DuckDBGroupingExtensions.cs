...
public static class DuckDBGroupingExtensions
{
    ...
    internal static readonly MethodInfo QuantileContArrayAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethods(BindingFlags.NonPublic | BindingFlags.Static)
            .Single(m => m.Name == nameof(QuantileContAggregate) && m.GetParameters().Length == 2 && m.GetParameters()[1].ParameterType == typeof(float[]));

    internal static readonly MethodInfo QuantileDiscAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethods(BindingFlags.NonPublic | BindingFlags.Static)
            .Single(m => m.Name == nameof(QuantileDiscAggregate) && m.GetParameters().Length == 2 && m.GetParameters()[1].ParameterType == typeof(double));

    internal static readonly MethodInfo QuantileDiscArrayAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethods(BindingFlags.NonPublic | BindingFlags.Static)
            .Single(m => m.Name == nameof(QuantileDiscAggregate) && m.GetParameters().Length == 2 && m.GetParameters()[1].ParameterType == typeof(float[]));
    ...
    internal static readonly MethodInfo CovarSampAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
    ...
    internal static readonly MethodInfo RegrAvgyAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
    ...
    internal static readonly MethodInfo RegrSxxAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(RegrSxxAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;

    internal static readonly MethodInfo RegrSxyAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(RegrSxyAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;

    internal static readonly MethodInfo RegrSyyAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(RegrSyyAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    internal static readonly MethodInfo KurtosisAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(KurtosisAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    internal static readonly MethodInfo MedianAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(MedianAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;

    internal static readonly MethodInfo ModeAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(ModeAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;

    internal static readonly MethodInfo SemAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(SemAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    internal static readonly MethodInfo StddevPopAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(StddevPopAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;

    internal static readonly MethodInfo StddevSampAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
    ...
    internal static readonly MethodInfo VarSampAggregateMethod
        = typeof(DuckDBGroupingExtensions).GetMethod(
            nameof(VarSampAggregate),
            BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static TResult QuantileCont<TKey, TSource, TResult>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TResult> selector,
        double pos)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(QuantileCont)));

    /// <summary>
    /// Translates to the DuckDB <c>QUANTILE_CONT</c> aggregate function, returning the interpolated quantiles
    /// of all non-null values selected in the group at the specified positions.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static TResult[] QuantileCont<TKey, TSource, TResult>(
    ...
    /// <summary>
    /// Translates to the DuckDB <c>QUANTILE_DISC</c> aggregate function, returning the discrete quantile
    /// of all non-null values selected in the group at the specified position.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static TResult QuantileDisc<TKey, TSource, TResult>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TResult> selector,
        double pos)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(QuantileDisc)));

    /// <summary>
    /// Translates to the DuckDB <c>QUANTILE_DISC</c> aggregate function, returning the discrete quantiles
    /// of all non-null values selected in the group at the specified positions.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static TResult[] QuantileDisc<TKey, TSource, TResult>(
    ...
    /// <summary>
    /// Translates to the DuckDB <c>COVAR_SAMP</c> aggregate function, returning the sample covariance for non-null pairs in a group.
    /// Returns <see langword="null" /> when the group contains no non-null pairs.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? CovarSamp<TKey, TSource, TY, TX>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TY> y,
        Func<TSource, TX> x)
    ...
    /// <summary>
    /// Translates to the DuckDB <c>REGR_AVGY</c> aggregate function, returning the average of the dependent variable
    /// <paramref name="y" /> for non-null pairs in a group (<c>sum(y)/count(y)</c>).
    /// Returns <see langword="null" /> when the group contains no non-null pairs.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? RegrAvgy<TKey, TSource, TY, TX>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TY> y,
        Func<TSource, TX> x)
    ...
    /// <summary>
    /// Translates to the DuckDB <c>REGR_SXX</c> aggregate function, returning <c>regr_count(y, x) * var_pop(x)</c>
    /// for non-null pairs in a group.
    /// Returns <see langword="null" /> when the group contains no non-null pairs.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? RegrSxx<TKey, TSource, TY, TX>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TY> y,
        Func<TSource, TX> x)
    ...
    /// <summary>
    /// Translates to the DuckDB <c>REGR_SXY</c> aggregate function, returning <c>regr_count(y, x) * covar_pop(y, x)</c>
    /// for non-null pairs in a group.
    /// Returns <see langword="null" /> when the group contains no non-null pairs.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? RegrSxy<TKey, TSource, TY, TX>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TY> y,
        Func<TSource, TX> x)
    ...
    /// <summary>
    /// Translates to the DuckDB <c>REGR_SYY</c> aggregate function, returning <c>regr_count(y, x) * var_pop(y)</c>
    /// for non-null pairs in a group.
    /// Returns <see langword="null" /> when the group contains no non-null pairs.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? RegrSyy<TKey, TSource, TY, TX>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TY> y,
        Func<TSource, TX> x)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(RegrSyy)));
    ...
    /// <summary>
    /// Translates to the DuckDB <c>KURTOSIS</c> aggregate function, returning the sample excess kurtosis
    /// of all non-null values selected in the group.
    /// Returns <see langword="null" /> when the group contains no non-null values.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? Kurtosis<TKey, TSource, TValue>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TValue> selector)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(Kurtosis)));
    ...
    /// <summary>
    /// Translates to the DuckDB <c>MEDIAN</c> aggregate function, returning the middle value of all
    /// non-null values selected in the group (or the average of the two middle values for even value counts).
    /// Returns <see langword="null" /> when the group contains no non-null values.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? Median<TKey, TSource, TValue>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TValue> selector)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(Median)));

    /// <summary>
    /// Translates to the DuckDB <c>MODE</c> aggregate function, returning the most frequent value (mode)
    /// of all non-null values selected in the group. When multiple values have the same frequency,
    /// returns the first encountered value.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static TResult Mode<TKey, TSource, TResult>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TResult> selector)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(Mode)));

    /// <summary>
    /// Translates to the DuckDB <c>SEM</c> aggregate function, returning the standard error of the mean
    /// of all non-null values selected in the group.
    /// Returns <see langword="null" /> when the group contains no non-null values.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? Sem<TKey, TSource, TValue>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TValue> selector)
    ...
    /// <summary>
    /// Translates to the DuckDB <c>STDDEV_POP</c> aggregate function, returning the population standard deviation
    /// of all non-null values selected in the group.
    /// Returns <see langword="null" /> when the group contains no non-null values.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? StddevPop<TKey, TSource, TValue>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TValue> selector)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(StddevPop)));

    /// <summary>
    /// Translates to the DuckDB <c>STDDEV_SAMP</c> aggregate function, returning the sample standard deviation
    /// of all non-null values selected in the group.
    /// Returns <see langword="null" /> when the group contains no non-null values.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? StddevSamp<TKey, TSource, TValue>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TValue> selector)
    ...
    /// <summary>
    /// Translates to the DuckDB <c>VAR_SAMP</c> aggregate function, returning the sample variance
    /// of all non-null values selected in the group.
    /// Returns <see langword="null" /> when the group contains no non-null values.
    /// Can only be used in LINQ queries; calling it on the client throws.
    /// </summary>
    public static double? VarSamp<TKey, TSource, TValue>(
        this IGrouping<TKey, TSource> source,
        Func<TSource, TValue> selector)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(VarSamp)));
    ...
    internal static TResult[] QuantileContAggregate<TResult>(IEnumerable<TResult> source, float[] pos)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(QuantileContAggregate)));

    internal static TResult QuantileDiscAggregate<TResult>(IEnumerable<TResult> source, double pos)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(QuantileDiscAggregate)));

    internal static TResult[] QuantileDiscAggregate<TResult>(IEnumerable<TResult> source, float[] pos)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(QuantileDiscAggregate)));
    ...
    internal static double? KurtosisAggregate<TValue>(IEnumerable<TValue> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(KurtosisAggregate)));
    ...
    internal static double? MedianAggregate<TValue>(IEnumerable<TValue> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(MedianAggregate)));

    internal static TSource ModeAggregate<TSource>(IEnumerable<TSource> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(ModeAggregate)));
    ...
    internal static double? StddevPopAggregate<TValue>(IEnumerable<TValue> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(StddevPopAggregate)));
    ...
    internal static double? VarSampAggregate<TValue>(IEnumerable<TValue> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(VarSampAggregate)));
    ...
    internal static double? RegrSxxAggregate<TY, TX>(IEnumerable<ValueTuple<TY, TX>> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(RegrSxxAggregate)));

    internal static double? RegrSxyAggregate<TY, TX>(IEnumerable<ValueTuple<TY, TX>> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(RegrSxyAggregate)));

    internal static double? RegrSyyAggregate<TY, TX>(IEnumerable<ValueTuple<TY, TX>> source)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(RegrSyyAggregate)));
